/**
 * Shared skill metadata used by both the server-side `/api/skills/local`
 * route (Node fs walker, dev only) and the client-side File System Access
 * reader (works on Vercel after the user authorizes a directory).
 *
 * Keep this file pure data + types — no Node, no DOM. Both contexts import it.
 */
...
export const UNCATEGORIZED_DEFAULTS: SkillCatalogEntry = {
  category: 'Other',
  categoryIcon: '📦',
  description: '',
};
